...
class Employee_Orientation(models.Model):
    ...
    def onchange_employee(self):
        ...
        if self.employee_id.join_date:
            ...
            print(into_date)
            self.date=into_date
            self.sevenday_mail=into_date+ timedelta(days=-7)
            print(self.sevenday_mail)
            self.oneday_mail=into_date+ timedelta(days=-1)
            print(self.oneday_mail)
    ...
    time = fields.Char('Reporting time')
    delivery_time = fields.Float(string='Time')
    sevenday_mail=fields.Date(string='Seven Day before mail')
    oneday_mail=fields.Date(string='One Day Before mail')
